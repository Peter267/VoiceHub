...
const setupHarmonyOSListeners = () => {
  ...
  const handleHarmonyOSNext = async () => {
    try {
      ...
      if (!success) {
        console.log('没有下一首电影或切换失败，继续播放当前电影')
        // 如果切换失败，不做任何操作，继续播放当前电影
      }
    } catch (error) {
      console.error('切换下一首电影失败:', error)
      // 切换失败时不停止播放，继续播放当前电影
    }
  }
  ...
  const handleHarmonyOSPrevious = async () => {
    try {
      ...
      if (!success) {
        console.log('没有上一首电影或切换失败，继续播放当前电影')
        // 如果切换失败，不做任何操作，继续播放当前电影
      }
    } catch (error) {
      console.error('切换上一首电影失败:', error)
      // 切换失败时不停止播放，继续播放当前电影
    }
  }
