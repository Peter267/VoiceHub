...
export function useAudioPlayer() {
  ...
  const playNext = async () => {
    if (currentPlaylist.value.length === 0 || currentPlaylistIndex.value === -1) {
      console.log('没有播放列表或当前电影不在播放列表中')
      return false
    }
      ...
      if (nextSong.musicPlatform && nextSong.musicId) {
        try {
          ...
          if (url) {
            ...
            console.warn(`跳过第${nextIndex + 1}首电影：无法获取播放链接，可能是付费内容`)
          }
        } catch (error) {
          console.warn(`跳过第${nextIndex + 1}首电影：获取播放URL失败`, error)
        }
      } else {
        console.warn(`跳过第${nextIndex + 1}首电影：缺少音乐平台或ID信息`)
      }
      ...
      if (prevSong.musicPlatform && prevSong.musicId) {
        try {
          ...
          if (url) {
            ...
            console.warn(`跳过第${prevIndex + 1}首电影：无法获取播放链接，可能是付费内容`)
          }
        } catch (error) {
          console.warn(`跳过第${prevIndex + 1}首电影：获取播放URL失败`, error)
        }
      } else {
        console.warn(`跳过第${prevIndex + 1}首电影：缺少音乐平台或ID信息`)
      }
