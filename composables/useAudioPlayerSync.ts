    ...
    try {
      ...
      if (!globalAudioPlayer.hasPrevious.value) {
        if (window.$showNotification) {
          window.$showNotification('没有上一首电影', 'warning')
        }
      ...
      if (success) {
        ...
        if (window.$showNotification) {
          window.$showNotification('播放上一首电影失败', 'error')
        }
      }
    } catch (error) {
      console.error('播放上一首电影失败:', error)
      if (window.$showNotification) {
        window.$showNotification('播放上一首电影失败', 'error')
      }
    }
  ...
  const playNext = async (song?: any) => {
    try {
      ...
      if (!globalAudioPlayer.hasNext.value) {
        if (window.$showNotification) {
          window.$showNotification('没有下一首电影', 'warning')
        }
        return { success: false, newSong: null }
      }
      ...
      if (success) {
        ...
        if (window.$showNotification) {
          window.$showNotification('播放下一首电影失败', 'error')
        }
      }
    } catch (error) {
      console.error('播放下一首电影失败:', error)
      if (window.$showNotification) {
        window.$showNotification('播放下一首电影失败', 'error')
      }
    }
